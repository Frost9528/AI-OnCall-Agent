...
up:
	...
	@echo "$(YELLOW)🚀 正在执行增量启动（自动补齐未运行的容器，如 Prometheus）...$(NC)"
	@docker-compose -f $(DOCKER_COMPOSE_FILE) up -d
	@echo ""
	@echo "$(YELLOW)⏳ 给予网络和容器依赖层 10 秒初始化缓冲...$(NC)"
	@sleep 10
	@echo "$(GREEN)✅ Docker 基础设施同步完毕！$(NC)"
	@echo "$(YELLOW)📋 当前运行中的所有相关组件状态:$(NC)"
	@docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
	@echo ""
	@echo "$(GREEN)🌐 基础设施访问清单:$(NC)"
	@echo "   Milvus (向量数据库): localhost:19530"
	@echo "   Attu (数据库视觉看板): http://localhost:8000"
	@echo "   MinIO (对象存储控制台): http://localhost:9001 (admin/minioadmin)"
	@if docker ps --format '{{.Names}}' | grep -q "prometheus"; then \
		echo "   Prometheus (监控系统): http://localhost:9090"; \
	fi
